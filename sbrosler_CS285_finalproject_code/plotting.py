...
def plot_erp_b3_gaussiansmooth(erp1, label1, erp2, label2, window, start_time, end_time):
...
def plot_relative_electrode_strength(incorrect_feedback, correct_feedback):
    ...
    try: 
        erp = np.max(np.mean(incorrect_feedback,axis=0) - np.mean(correct_feedback, axis=0), axis=0)
